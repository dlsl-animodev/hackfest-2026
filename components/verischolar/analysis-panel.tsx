            ...
            <div className="rounded-[2rem] border border-[var(--line)] bg-[rgba(255,255,255,0.74)] p-5 shadow-[var(--shadow-soft)]">
                ...
                {actionState.analysis ? (
                    <div className="mt-5 space-y-4">
                        <div className="rounded-[1.5rem] border border-[var(--line)] bg-[rgba(255,251,243,0.86)] p-4">
                            ...
                            <p className="mt-3 text-xs text-[var(--muted)]">
                                Generated{" "}
                                {new Date(
                                    actionState.analysis.generatedAt,
                                ).toLocaleString()}
                            </p>
                        </div>
                        ...
                        <div className="rounded-[1.5rem] border border-[var(--line)] bg-[rgba(255,255,255,0.68)] p-4">
                            ...
                            <div className="mt-3 space-y-3">
                                {actionState.analysis.conflicts.length > 0 ? (
                                    actionState.analysis.conflicts.map(
                                        (conflict) => (
                                            <div
                                                key={conflict.claim}
                                            ...
                                            >
                                                ...
                                                <p className="mt-2 text-xs text-[var(--muted)]">
                                                    Supporting:{" "}
                                                    {conflict.supportingSourceIds.join(
                                                        ", ",
                                                    ) || "None"}
                                                </p>
                                                <p className="mt-1 text-xs text-[var(--muted)]">
                                                    Opposing:{" "}
                                                    {conflict.opposingSourceIds.join(
                                                        ", ",
                                                    ) || "None"}
                                                </p>
                                            </div>
                                    ...
                                    <p className="text-sm leading-7 text-[var(--muted)]">
                                        No sharp contradictions were detected
                                        yet. Add more varied sources to
                                        stress-test the claim landscape.
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid gap-4 lg:grid-cols-2">
                            <div className="rounded-[1.5rem] border border-[var(--line)] bg-[rgba(255,255,255,0.68)] p-4">
                                ...
                                <ul className="mt-3 space-y-2 text-sm leading-7 text-[color:rgba(61,47,34,0.92)]">
                                    {actionState.analysis.researchGaps.map(
                                        (gap) => (
                                            <li key={gap}>- {gap}</li>
                                        ),
                                    )}
                            ...
                            <div className="rounded-[1.5rem] border border-[var(--line)] bg-[rgba(255,255,255,0.68)] p-4">
                                ...
                                <ul className="mt-3 space-y-2 text-sm leading-7 text-[color:rgba(61,47,34,0.92)]">
                                    {actionState.analysis.confidenceNotes.map(
                                        (note) => (
                                            <li key={note}>- {note}</li>
                                        ),
                                    )}
                        ...
                        <Link href="/validate-gap">
                            <button className="inline-flex items-center gap-2 rounded-full bg-[var(--ink)] px-4 py-3 text-sm text-[var(--bg)] shadow-[0_18px_44px_rgba(33,21,13,0.16)] transition-transform duration-300 hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-50">
                                <SparkIcon className="h-4 w-4" />
                    ...
                    <div className="mt-5 rounded-[1.5rem] border border-dashed border-[var(--line-strong)] bg-[rgba(255,251,243,0.68)] p-5 text-sm leading-7 text-[var(--muted)]">
                        Select three or more sources and run synthesis to
                        generate a board-level summary, conflict map, and
                        research gap check.
                    </div>
                )}
            </div>
