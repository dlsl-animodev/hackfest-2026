...
export default function ValidateGapBoard({
  ...
  const resolvedAnalysis = initialAnalysis ?? analysisState?.analysis ?? null;
  ...
  useEffect(() => {
    if (resolvedAnalysis && iterations.length === 0) {
      setIterations([
        {
          id: 1,
          gaps: resolvedAnalysis.researchGaps,
          queries: resolvedAnalysis.validateGapSearch,
          logs: [],
          winner: null,
          exhausted: false,
        },
  ...
  }, [resolvedAnalysis, iterations.length]);
  ...
  async function startAgent() {
    ...
    for (const query of currentIter.queries) {
      ...
      setIterations((prev) => {
        ...
        next[currentIdx] = {
          ...next[currentIdx],
          logs: [...next[currentIdx].logs, { query, status: "searching" }],
        };
        return next;
      });
  ...
  return (
    <div>
      ...
      <div
        className={`mx-auto w-full space-y-10 ${embedded ? "max-w-none py-1" : "max-w-3xl py-10"}`}
      >
        {iterations.map((iteration, index) => {
          ...
          const resolvedSources = iteration.winner
            ? iteration.winner.searchResponse.sources.filter((s) =>
                iteration.winner!.evaluation.resolvingSourceIds.includes(s.id),
              )
            : [];
          const sourcesToDisplay =
            resolvedSources.length > 0
              ? resolvedSources
              : iteration.winner?.searchResponse.sources.slice(0, 3) || [];

          return (
            <div
              key={iteration.id}
              className={`${embedded ? "space-y-6 pb-8" : "space-y-8 border-b-2 border-dashed border-gray-200 pb-12 last:border-0"}`}
            >
              <div
                className={`flex flex-col ${embedded ? "gap-5 rounded-none border-0 bg-transparent px-0 py-0 shadow-none" : "gap-4"}`}
              >
                <section>
                  <div className="flex items-center justify-between">
                    <h1
                      className={`${embedded ? "text-3xl sm:text-[2.1rem]" : "text-2xl"} text-gray-900 type-display`}
                    >
                ...
                <ul
                  className={`${embedded ? "list-disc pl-6 text-base leading-8" : "list-disc pl-5 text-sm"} text-gray-700`}
                >
                  {iteration.gaps.map((gap: string, i: number) => (
                    <li key={i}>{gap}</li>
                  ))}
                </ul>
                ...
                {isLatest && !hasStarted && (
                  <Button
                    ...
                    className={`${embedded ? "w-fit px-6 py-3.5 text-base" : "w-fit"}`}
                  >
                    <Search />
                    {isAgentRunning
                      ? "Agent is running..."
                ...
                <Card className="flex flex-col gap-4">
                  <section>
                    ...
                    <p className="mt-2 text-sm text-gray-700">
                      The agent found literature resolving these gaps using the
                      query:{" "}
                      <strong>
                        &quot;
                        {iteration.winner.queryUsed}
                  ...
                  {isLatest && (
                    <div className="mt-4 flex items-center justify-between border-t pt-4">
                      <p className="text-sm font-medium text-gray-800">
                        Dig deeper into these papers to find the *next* layer of
                        gaps?
                      </p>
                      <Button
              ...
              {iteration.exhausted && (
                <Card className="flex flex-col gap-4">
                  ...
                  <p className="text-sm text-gray-700">
                    The agent exhausted all validation queries. Based on
                    provider data, these research gaps are genuinely unaddressed
                    in current literature. You have found a solid research
                    opportunity.
                  </p>
                </Card>
              )}
